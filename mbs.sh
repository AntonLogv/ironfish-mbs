...
function MintFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:mint --name="${GRAFFITI}" --metadata="${GRAFFITI}"  --amount=${RANDOM_MINT} --fee=$fee | tr -d '\0')
...
function BurnFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:burn --assetId="${IDENTIFIER}" --amount="${BURN_AMOUNT}" --fee=$fee | tr -d '\0')
...
function SendFunc() {
    ...
    RESULT=$(echo "Y" | ${BIN} wallet:send --assetId="${IDENTIFIER}" --amount="${SEND_AMOUNT}" --to="${ADDRESS_TO_SEND}" --memo="${GRAFFITI}" --fee=$fee | tr -d '\0')
...
function MainFunc() {
    ...
    GRAFFITI=$(echo $(${BIN} config:get blockGraffiti) | sed 's/\"//g')
       ...
       if [[ $(echo "$(GetBalanceFunc) > $min_balance" | bc ) -eq 1 ]]; then
        ...
        echo -e "$(PrintTime) not enough balance. minimum required: \$IRON $min_balance, but you have only: \$IRON $(GetBalanceFunc). if it's a bug, try in a few minutes.\n"
    fi
